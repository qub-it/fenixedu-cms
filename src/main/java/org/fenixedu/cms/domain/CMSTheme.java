...
public class CMSTheme extends CMSTheme_Base {
    ...
    public static CMSTheme getDefaultTheme() {
        return Bennu.getInstance().getCMSThemesSet().stream().filter(x -> x.isDefault()).findAny()
                .orElseThrow(() -> new RuntimeException("There is no default theme"));
    }
    ...
    public String getAssetsPath() {
        return CoreConfiguration.getConfiguration().applicationUrl() + "/api/cms/assets/" + getType() + "/"
    ...
    public Stream<Site> getAllSitesStream() {
        return Bennu.getInstance().getSitesSet().stream().filter(site -> getType().equals(site.getThemeType()));
    }
