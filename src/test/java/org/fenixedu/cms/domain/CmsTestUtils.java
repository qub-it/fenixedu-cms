...
public class CmsTestUtils {
    ...
    public static void setUserAsManager(final User user) {
    ...
    public static LocalizedString createLocalizedString(final String base) {
        return new LocalizedString(I18N.getLocale(), base);
    }

    public static User createAuthenticatedUser(final String username) {
        return createAuthenticatedUser(username, "name", "familyName", "mail@fenixedu.org");
    }

    public static User createAuthenticatedUser(final String username, final String name, final String familyName,
            final String email) {
    ...
    public static Site createSite(final User user, final String methodName) {
        LocalizedString siteName =
                new LocalizedString(Locale.UK, "site name uk " + methodName).with(Locale.US, "site name us " + methodName);
        LocalizedString siteDescription = new LocalizedString(Locale.UK, "site description uk " + methodName).with(Locale.US,
                "site description us " + methodName);
    ...
    public static Post createPost(final Site site, final String methodName) {
    ...
    public static PostContentRevision createVersion(final Post post, final String methodName) {
    ...
    public static Page createPage(final Site site, final String methodName) {
    ...
    public static Category createCategory(final Site site, final String methodName) {
        LocalizedString categoryName = new LocalizedString(Locale.UK, "category name uk " + methodName).with(Locale.US,
                "category name us " + methodName);
        Category category = new Category(site, categoryName);

        return category;
    }

    public static Menu createMenu(final Site site, final String methodName) {
    ...
    public static MenuItem createMenuItem(final Menu menu, final String methodName) {
        MenuItem menuItem = new MenuItem(menu);
        LocalizedString menuItemName = new LocalizedString(Locale.UK, "menuItem name uk " + methodName).with(Locale.US,
                "menuItem name us " + methodName);
        menuItem.setName(menuItemName);

        return menuItem;
    }

    public static CMSTheme createTheme(final String methodName) {
    ...
    public static RoleTemplate createRoleTemplate(final String name, final EnumSet<Permission> permissions) {
    ...
    public static EnumSet<Permission> bootstrapAdminPermissions() {
        return EnumSet.of(Permission.CREATE_POST, Permission.CREATE_PAGE, Permission.SEE_POSTS, Permission.SEE_PRIVATE_POSTS,
                ...
                Permission.EDIT_SITE_INFORMATION, Permission.LIST_CATEGORIES, Permission.EDIT_CATEGORY,
                Permission.DELETE_CATEGORY, Permission.CREATE_CATEGORY, Permission.MANAGE_ANALYTICS, Permission.MANAGE_ROLES,
                Permission.PUBLISH_PAGES, Permission.PUBLISH_POSTS, Permission.PUBLISH_SITE, Permission.CREATE_MENU,
                Permission.DELETE_MENU, Permission.LIST_MENUS, Permission.EDIT_MENU, Permission.CREATE_MENU_ITEM,
                Permission.DELETE_MENU_ITEM, Permission.EDIT_MENU_ITEM, Permission.CHANGE_PATH_PAGES,
                Permission.CHOOSE_PATH_AND_FOLDER, Permission.EDIT_SITE_INFORMATION, Permission.CHOOSE_DEFAULT_PAGE);
    }
